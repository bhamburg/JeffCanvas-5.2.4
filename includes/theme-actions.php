...
function woo_custom_styling() {
	...
	if ( isset( $woo_options['woo_style_disable'] ) && $woo_options['woo_style_disable'] != 'true' ) {} else {
		...
		if ( $font_post_title )
			$output .= '.page-title, .post .title, .page .title, .post .title a:link, .post .title a:visited, .page .title a:link, .page .title a:visited {'.woo_generate_font_css( $font_post_title, 1.1 ).'}' . "\n";
		if ( $font_post_meta )
		...
		if ( is_singular() ) {
			...
			if ( $layout != '' ) {
				$woo_options['woo_layout'] = $layout;
					 ...
					 ( class_exists( 'woocommerce' ) && function_exists( 'is_shop' )  && function_exists( 'is_product' )  && function_exists( 'is_cart' ) && ( is_shop() || is_product() || is_cart() || is_checkout() ) ) ) {
...
if ( ! function_exists( 'woo_slider_magazine' ) ) {
	...
	if (is_page('kit')) {
		$query = new WP_Query( array( 'tag' => 'featured-kit' , 'posts_per_page' => $args['posts_per_page'] , 'cat' => 4 ) );
	}
	else {
...
<?php 
// Add KIT banner if it is a KIT article
// References: 	http://codex.wordpress.org/Category_Templates
//				http://codex.wordpress.org/Conditional_Tags
if (is_page('kit') || is_page('kit-signup')) : ?>
	<div id="kit-banner">
		<div id="kit-logo">
			<a class="kit-logo" href="/atjeff/kit/"><span>Keep In Touch</span></a>
		</div>
		<div id="kit-button">
			<a class="kit-button lbpModal" href="http://eepurl.com/eheBD"><span>Sign Up Today!</span></a>
			<a class="kit-button-text lbpModal" href="http://eepurl.com/eheBD">Sign up for our monthly <em>Keep In Touch</em> newsletter:</a>
		</div>
		<div style="clear: both"></div>
	</div>
<?php elseif (is_category('Keep in Touch')) : ?>
	<div id="kit-banner">
		<div id="kit-logo">
			<a class="kit-logo" href="/atjeff/kit/"><span>Keep In Touch</span></a>
		</div>
...
<?php elseif (is_single() && in_category('Keep in Touch')) : ?>
	<div id="kit-banner">
		<div id="kit-logo">
			<a class="kit-logo" href="/atjeff/kit/"><span>Keep In Touch</span></a>
		</div>
